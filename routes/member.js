...
function PubClient(myid) {
    ...
    console.log("client");
    console.log(myid);
    var client = con.coll.findOne({
        _id: myid
    })
    client.pubcoll = con.pubcoll.find({
        client_id: myid
...
exports.memberAdvertise = function(req, res, key) {
    ...
    if (id == "" || id == 0 || id == null || id.length < 24) {
        ...
        con.coll.find({
            _id: ObjectId(id)
        }).toArray(function(err, clientDocs) {
            console.log(clientDocs);
            res.render('memberAdvertise', {
                title: "Member Advertise",
                doc: clientDocs
            });
        })
    ...
    if (id == "" || id == 0) {
        ...
        con.coll.find({
            ...
            if (err) {
                ...
                var uniqueList = _underscore.uniq(clientDocs[0].quizz, function(item, key, quizztitle) {
